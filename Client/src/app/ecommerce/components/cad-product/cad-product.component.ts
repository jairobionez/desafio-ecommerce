		...
		if (this.form.valid) {
			...
			this._ecommerceService
				...
				.pipe(
					...
					catchError(err => {
						this._snackBar.open("Não foi possível deletar o produto","Fechar",{duration: 3000});
						return throwError(err);
					})
				)
				.subscribe(response => {
					this._snackBar.open("Produto deletado com sucesso", "Fechar", {duration: 3000});});
		} else {
			this._snackBar.open(
				"Formulária inválido, por favor verifique os campos e imagem","Fechar",{duration: 3000});
		}
	}
		...
		if (this.form.valid) {
			...
			this._ecommerceService
				...
				.pipe(
					...
					catchError(err => {
						this._snackBar.open("Não foi possível atualizar o produto", "Fechar", { duration: 3000 });
						return throwError(err);
					})
				)
				.subscribe(response => {
